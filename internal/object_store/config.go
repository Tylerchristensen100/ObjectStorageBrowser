...
func LoadConfig(app AppContext, endpoint, key, password string) (*ObjectStore, error) {
	...
	if err != nil {
		return nil, errors.New("failed to create S3 client: " + err.Error())
	}
...
func (o *ObjectStore) bucketExists(app AppContext, bucket string) (bool, error) {
	...
	if err != nil {
		app.Logger().Error("object_storage/config: error checking if bucket exists", "error", err)
		return false, errors.New("error checking if bucket exists")
	}
	if !exists {
		app.Logger().Error("object_storage/config: bucket does not exist", "bucket", bucket)
