...
func (o *ObjectStore) GetFile(app AppContext, bucket string, path string) ([]byte, error) {
	...
	if err != nil {
		app.Logger().Error("object_storage/document: error getting object from storage", "error", err)
		return nil, err
	}
	...
	if _, err := buf.ReadFrom(object); err != nil {
		app.Logger().Error("object_storage/document: error reading object data", "error", err)
		return nil, err
	}
